...
export async function GET(req: Request) {
	...
	if (!code || !state || state !== cookieState)
		return fail("Google 登入驗證失敗");

	const clientId = process.env.GOOGLE_CLIENT_ID;
	const clientSecret = process.env.GOOGLE_CLIENT_SECRET;
	if (!clientId || !clientSecret) return fail("Google 登入尚未設定");
	...
	try {
		...
		if (!r.ok) return fail("Google 權杖交換失敗");
		tokens = await r.json();
	} catch {
		return fail("無法連線 Google");
	}
	if (!tokens.id_token) return fail("Google 未回傳身分權杖");
	...
	if (!email || !googleId) return fail("Google 帳號缺少 email");
